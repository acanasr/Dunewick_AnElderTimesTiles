	...
	public static class HierarchyFolderUtility
	{
		...
		private static void CheckForAndRemoveHierarchyFoldersInChildren([NotNull]Transform transform, StrippingType strippingType, bool destroyImmediate, List<GameObject> setChildrenActiveDelayed)
		{
			...
			if(hierarchyFolder != null)
			{
				...
				#if DEV_MODE || ASSERT_COMPONENT_COUNT
				if(HasSupernumeraryComponents(hierarchyFolder))
				{
					Debug.LogError("HierarchyFolder " + transform.name + " contained " + transform.GetComponents<Component>().Length + " components! All components will be destroyed along with the Hierarchy Folder.");
				}
				#endif

				#if DEV_MODE || ASSERT_CHILD_COUNT
